...
void core::archive::sticker_data::serialize(icollection* _collection)
{
	...
	boost::split(components, id_, boost::is_any_of(":"));
...
void core::archive::voip_data::serialize(coll_helper &_coll) const
{
...
void core::archive::voip_data::serialize(core::tools::tlvpack &_pack) const
{
...
void file_sharing_data::serialize(icollection* _collection, const std::string &_internal_id, const bool _is_outgoing) const
{
	assert(_collection);
...
void file_sharing_data::serialize(core::tools::tlvpack& _pack) const
{
	assert(_pack.empty());
	...
	const bool _is_outgoing) const
{
	...
	if (has_mchat_members())
	{
		serialize_mchat_members(coll);
	}

	if (has_chat_modifications())
	{
		serialize_chat_modifications(coll);
	}
...
void chat_event_data::serialize(tools::tlvpack &_pack) const
{
	...
	if (has_mchat_members())
	{
		serialize_mchat_members(_pack);
	}

	if (has_chat_modifications())
	{
		serialize_chat_modifications(_pack);
	}
...
void chat_event_data::serialize_chat_modifications(coll_helper &_coll) const
{
	if (type_ == chat_event_type::chat_name_modified)
	{
...
void chat_event_data::serialize_chat_modifications(tools::tlvpack &_pack) const
{
	if (type_ == chat_event_type::chat_name_modified)
	{
		const auto &new_name = chat_.new_name_;
...
void chat_event_data::serialize_mchat_members(coll_helper &_coll) const
{
...
void chat_event_data::serialize_mchat_members(tools::tlvpack &_pack) const
{
...
void history_message::serialize(core::tools::binary_stream& _data) const
{
	...
	if (file_sharing_)
	{
		...
		file_sharing_->serialize(file_sharing_pack);
	...
	if (chat_event_)
	{
		...
		//chat_event_->serialize(chat_event_pack);
...
int32_t history_message::unserialize(core::tools::binary_stream& _data)
{
	...
	for (auto tlv_field = msg_pack.get_first(); tlv_field; tlv_field = msg_pack.get_next())
	{
		...
		case message_fields::mf_sender_friendly:
			...
			break;
		case  message_fields::mf_voip_sender_aimid:
		{
			auto someValue = tlv_field->get_value<std::string>(std::string());
		}
		break;
		case message_fields::mf_chat_sender:
		{
			auto someValue = tlv_field->get_value<std::string>(std::string());
		}
		break;
		case message_fields::mf_text:
		{
			...
			//auto currId = chat_event_->getSenderId();
		}
		break;
		...
		break;
		case message_fields::mf_chat_event_sender_aimid:
		{
			const auto pack = tlv_field->get_value<core::tools::tlvpack>();
			chat_event_ = chat_event_data::make_from_tlv(pack);
		}
...
//	bool parse_new_file_sharing_uri(const std::string &_uri, std::string &_fileId)
//	{
//		assert(!_uri.empty());
//
...
//	}
	...
	void serialize_metadata_from_uri(const std::string &uri, coll_helper &coll)
	{
		assert(!uri.empty());

		std::string id;

		//if (!tools::parse_new_file_sharing_uri(uri, id))
		//{
		//	return;
		//}
	...
	//void find_person(const std::string &_aimid, const persons_map &_persons, std::string &_friendly)
	//{
	//	assert(!_aimid.empty());
	...
	//	if (iter_p != _persons.end())
	//	{
	//		_friendly = iter_p->second.friendly_;
	//	}
	//	else
	//	{
	//		_friendly = _aimid;
	...
	std::set<std::string> read_members(const rapidjson::Value &_parent)
	{
